...
import string
import tracemalloc
from suffixArray import suffixList_v1, suffixList_v2, sufPosition
...
def run_experiments(version='v2'):
    ...
    query_counts = [1]   # Se pueden agregar otros conteos: 10_000, 100_000, 1_000_000, etc.
    ...
    print(f"{'Tamaño texto':<15} {'Num consultas':<15} {'Tiempo SA (s)':<15} {'Memoria SA (MB)':<15} {'Tiempo consultas (s)':<20} {'Memoria consultas (MB)':<20} {'Tiempo total (s)':<15}")
    print("-" * 80)
    ...
    for text_size in text_sizes:
        ...
        if version == 'v2':
            base_array = suffixList_v2(text)

        suffix_array = base_array
        t1 = time.perf_counter()
        sa_time = t1 - t0
        sa_memory = tracemalloc.get_traced_memory()[1] / (1024 * 1024)  # Convertir a MB
        tracemalloc.stop()
        ...
        for qc in query_counts:
            ...
            query_memory = tracemalloc.get_traced_memory()[1] / (1024 * 1024)  # Convertir a MB
            tracemalloc.stop()
            
            total_time = sa_time + query_time
            print(f"{text_size:<15} {qc:<15} {sa_time:<15.3f} {sa_memory:<15.2f} {query_time:<20.3f} {query_memory:<20.2f} {total_time:<15.3f}")    
    print("=" * 80)
